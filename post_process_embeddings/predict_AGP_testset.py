...
emb_txt = "/glove_input_0_emb.txt"
emb_fasta = "/transformed_glove_input_0_emb.txt"

#Load data
seed = "0"
f = open(data_dir + "/otu_train_" + str(seed) + ".obj", "rb")
otu_train = pickle.load(f)
f.close()

f = open(data_dir + "/otu_test_" + str(seed) + ".obj", "rb")
otu_test = pickle.load(f)
f.close()

f = open(data_dir + "/map_train_" + str(seed) + ".obj", "rb")
map_train = pickle.load(f)
f.close()

f = open(data_dir + "/map_test_" + str(seed) + ".obj", "rb")
map_test = pickle.load(f)
f.close()

# embedding matrix and corresponding nucleotide sequences of ASVs
